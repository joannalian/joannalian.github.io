...
function buildImportPlan(rows,headers=[]){
  ...
  const existingMap=new Map();state.groups.forEach(g=>{const key=normalizeGroupName(g.name),list=existingMap.get(key)||[];list.push(g);existingMap.set(key,list)});const duplicateExisting=[...existingMap.values()].filter(list=>list.length>1);if(duplicateExisting.length)issues.push(`目前資料已有重複群組：${duplicateExisting.map(list=>list[0].name).join('、')}`);
  const activeRows=named.filter(r=>r.count>0),activeMap=new Map(activeRows.map(r=>[normalizeGroupName(r.name),r])),additions=[],updates=[],unchanged=[],removals=[];
  activeRows.forEach(row=>{const current=existingMap.get(normalizeGroupName(row.name))?.[0];if(!current){additions.push(row);return}const changes=[];if(current.members.length!==row.count)changes.push(`人數 ${current.members.length}→${row.count}`);if(current.category!==row.category)changes.push(`分類 ${current.category}→${row.category}`);if((current.note||'')!==row.note)changes.push('備註');if((current.cnCake||0)!==row.cnCake||(current.westCake||0)!==row.westCake)changes.push('喜餅');const child=current.members.filter(m=>m.child).length,veg=current.members.filter(m=>m.veg).length;if(child!==row.child)changes.push(`兒童椅 ${child}→${row.child}`);if(veg!==row.veg)changes.push(`素食 ${veg}→${row.veg}`);const removedMembers=row.count<current.members.length?[...current.members].sort((a,b)=>(b.originalIndex||0)-(a.originalIndex||0)).slice(0,current.members.length-row.count):[];(changes.length?updates:unchanged).push({group:current,row,changes,seatedRemoved:removedMembers.filter(m=>m.tableId).length})});
  state.groups.forEach(group=>{if(!activeMap.has(normalizeGroupName(group.name)))removals.push({group,seated:group.members.filter(m=>m.tableId).length,explicitZero:incomingMap.get(normalizeGroupName(group.name))?.[0]?.count===0})});
  return {issues,additions,updates,unchanged,removals,zeroRows:named.filter(r=>r.count===0),activeRows};
}
function parseWorkbook(file){const reader=new FileReader();reader.onload=()=>{try{const wb=XLSX.read(reader.result,{type:'array'}),ws=wb.Sheets[wb.SheetNames[0]],matrix=XLSX.utils.sheet_to_json(ws,{header:1,defval:''}),headers=(matrix[0]||[]).map(v=>String(v).trim()),raw=XLSX.utils.sheet_to_json(ws,{defval:''});importRows=raw.map(importRow).filter(r=>r.name);importPlan=buildImportPlan(importRows,headers);renderImportPreview()}catch(e){console.error(e);toast('Excel 無法讀取，請確認欄位與檔案格式')}};reader.readAsArrayBuffer(file)}
...
function renderImportPreview(){
  ...
  const updates=importList('更新群組',plan.updates.map(x=>`<p><b>${esc(x.group.name)}</b><span>${esc(x.changes.join('、'))}${x.seatedRemoved?`；將移除 ${x.seatedRemoved} 位已入桌成員`:''}</span></p>`),'sync-update');
  const removals=importList('移除群組',plan.removals.map(x=>`<p><b>${esc(x.group.name)}</b><span>${x.explicitZero?'Excel 人數為 0':'最新 Excel 已無此群組'}${x.seated?`；移除 ${x.seated} 位已入桌成員`:''}</span></p>`),'sync-remove');
